...
def piv_readin(base_name_input, data_sets, sizex, sizey):
    ...
    for i in x_range:
        ...
        temp_x = np.array(np.values.reshape(temp['Xlocation (mm)'], (sizex, sizey)))
        temp_y = np.array(np.values.reshape(temp['Ylocation (mm)'], (sizex, sizey)))
        temp_u[count] = np.array(np.values.reshape(temp['U (m/sec)'], (sizex, sizey)))
        temp_v[count] = np.array(np.values.reshape(temp['V (m/sec)'], (sizex, sizey)))
        count+=1
